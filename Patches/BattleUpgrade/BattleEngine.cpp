...
extern "C" b32 LoadDll(const char* dllName)
{
    ...
    // BUG upstream: array FULL → the loop ended without loading anything and returned 1 = "success" →
    // the caller jumped to the EventAdd of a module that was NEVER loaded → data abort → freeze.
    // Now 0: the caller skips the effect instead of crashing.
    return 0;
}
static b32 IsPinnedDll(const char* name);   // see the definition above FreeLoadedDlls

extern "C" void FreeDll(const char* dllName) {
    if (dllName == nullptr)
        return;

    // W1: the Moves-DLLs are PINNED — never released, so they never move in the heap and the
    // pointer that the BattleUpgrade keeps in its table stays valid (the RPM linker does not re-resolve
    // imports that are already resolved). See the extended comment above FreeLoadedDlls.
    if (IsPinnedDll(dllName))
        return;
...
// ── W1: "PINNED" DLLs — loaded once and NEVER released ─────────────────────────────────────
// The runtime-moves freeze is not the LOAD, it's the RELOCATION: the DLL is released and then
// reloaded at a DIFFERENT address (the PMC heap reuses the freed slots), while the pointer that the
// BattleUpgrade keeps in its table (moveEventAddTableExt[].func) stays at the OLD address — because the
// RPM linker REFUSES to re-resolve imports that are already resolved:
//     Module::ImportModule(): if (ALL_IMPORTED || !MODULE_LINK_READY) return 0;
// → it calls into the void (metadata of another module) → data abort → freeze. Tested with 2 savestates:
// FellStinger @0x02236428 (code ..0x022364E0) → moved to 0x02236668, and the crash PC was
// 0x022364D0 = inside the old range.
//
// If the DLL is NEVER released, the address never changes and the pointer stays valid. And since
// the BattleUpgrade is now NO longer resident, it is reloaded and RE-LINKED at every battle against the
// DLL still in memory → pointer always correct. The two things together close the loop.
// Cost: ~23 move-modules of a few hundred bytes sitting in the heap — trivial in the 1408KB.
// NB: kDLL keeps ITS OWN list (`_FindLibrary`), independent from loadedDlls[] → a LoadLibrary of a
// module already loaded FINDS it again and does not reload it.
// ── What gets PINNED, and why it depends on the console ──────────────────────────────────────────
// MOVES ARE ALWAYS PINNED, on BOTH consoles: it's the bug fix — the Moves-DLL must NEVER
// move, otherwise the pointer in moveEventAddTableExt[] becomes stale (the RPM linker does not
// re-resolve imports that are already resolved). It costs nothing: FellStinger = 564 B loaded. Verified in DS mode
// from the user's savestate: the module is there, it is retained, and the move was used several times without freeze.
//
// ABILITIES instead are pinned ONLY on DSi. They have in theory the same defect, but:
//   · they never gave problems (they load at battle start in stable order → same address);
//   · they are 133 modules ≈ 111KB. On DSi the heap is 1408KB → free. On **DS the heap is 142KB** and, MEASURED in
//     battle, only **36.6KB free** remain (BattleUpgrade 82.6 fixed + NitroKernel 15.9 +
//     FairyPatch 4.8 + Base 3.3) → pinning them all would fill it (~40 distinct abilities and you're out).
// On DS they therefore stay released, exactly as they have always done: the DS loses nothing.
//
// DSi/DS discriminator: `os_MemRegionEnds` @0x02FFFDC4. In DS mode PMC LOWERS it to the base of its own
// heap (measured: 0x023BC870); on DSi it does NOT touch it (takes the window below the ARM7) and it stays the game's
// value (measured: 0x02F80000). So `> 0x02400000` ⇒ DSi. Verified with GDB on both.
static b32 IsDSiMode() {
    return *(volatile u32*)0x02FFFDC4 > 0x02400000;
}
static b32 IsMovesDll(const char* name) {
    return name && name[0] == 'M' && name[1] == 'o' && name[2] == 'v'
        && name[3] == 'e' && name[4] == 's' && name[5] == '/';
}
static b32 IsPinnedDll(const char* name) {
    if (!name) return 0;
    if (IsMovesDll(name)) return 1;   // moves: ALWAYS, on both consoles (it's the fix)
    return IsDSiMode();               // the rest: only on DSi, where the heap allows it
}
...
extern "C" void FreeLoadedDlls() {
    ...
    for (u8 dllIdx = 0; dllIdx < MAX_LOADED_DLLS; ++dllIdx)
    {
        ...
        if (!IsPinnedDll(loadedDlls[dllIdx].name))   // ← the pinned ones are NEVER released
            k::dll::ReleaseLibrary(loadedDlls[dllIdx].name);
#if LIBRARY_LOAD_DEBUG
        DPRINTF("Freed %s!\n", loadedDlls[dllIdx]);
#endif
...
// W1DIAG: static buffer in the DLL to bypass the heap allocation in BattleField_Init
// (test: if the crash was the alloc from the W1 battle-heap, with this it disappears). Revert afterwards.
static BattleFieldExt s_battleFieldStatic;
...
extern "C" void THUMB_BRANCH_SAFESTACK_AddExpAndEVs(ServerFlow * serverFlow, BattleParty * party, BattleMon * defeatedMon, CalcExpWork * partyCalcExpWork) {
    ...
    for (u32 i = 0; i < monsInParty; i = ++i) {
        ...
        if (calcExpWork->level) {
            ...
            u8 lvlCap = (*capPtr == 0) ? 100 : (u8)*capPtr;
...
MegaEvolution megaevolutions[]{
    // ★ W1 MECHANIC-TEST — Darmanitan → form 1 = Zen Mode, which REALLY EXISTS in the W1 personal
    // (a/0/1/6 entry 555: formeDataOffs=666, formeCount=2). All the upstream ITEM_*ITE are still
    // `#define 1` = MASTER BALL (the megastones don't exist), so with the Master Ball in hand you
    // observe the PURE mechanic — ring message, real form change, ability swap,
    // speed-order recalc — instead of the corruption that an inexistent form would give.
    // Exact same test passed on White 2. Remove when the real mega-forms exist.
    {PKM_DARMANITAN, ITEM_MEGA_RING, 1},
    {PKM_GARCHOMP, ITEM_MEGA_RING, 1},   // Mega Garchomp: form 1 = personal 669, sprite block 652+26=678
    ...
    // Upstream bugs fixed (Jul 12): the line {PKM_MEWTWO, ITEM_MEWTWONITE_X, 1} was DUPLICATED (it's
    // already above) and the last one was {PKM_NULL, ...} with PKM_NULL = 0 → a species inexistent in the table.
    // Removed both: they were harmless only by chance (the scan is linear and stops at the first match).
};
...
// ── W1: START input without GCTX_HIDGetPressedKeys ────────────────────────────────────────────────
// On W1 that function was never found (the GFL HID chain is redesigned) and it is STUBBED
// in the ESDB to a `movs r0,#0; bx lr` → always returns 0 → START would NEVER have fired and the mega
// would have been untriggerable. No need to find it: we read directly the key register
// of the hardware, KEYINPUT @0x04000130 (ACTIVE LOW: bit at 0 = key pressed), and we do the
// EDGE detection ourselves — "pressed now, not pressed before" — which is exactly what the toggle
// requires (otherwise, holding it down, it would turn on and off every frame).
// The KEYINPUT bit layout (A=1, B=2, Select=4, Start=8, R=0x10, L=0x20, Up=0x40, Down=0x80,
// R=0x100, L=0x200) MATCHES swan's InputButton enum on bits 0-9 → KEY_START = 0x8 holds for
// both, no translation. (X/Y are on the ARM7, but we don't need them.)
// s_prevKeys is a BattleUpgrade global, which is NO longer resident → it zeroes clean at every
// battle, without needing explicit resets. See [[pw2-w1-resident-stale-imports]].
static u16 s_prevKeys = 0;
static u32 W1_GetPressedKeys()
{
    u16 cur = (u16)(~(*(volatile u16*)0x04000130)) & 0x3FF;
    u16 pressed = (u16)(cur & ~s_prevKeys);
    s_prevKeys = cur;
    return pressed;
}
...
{
    // We sample ALWAYS, even when the flag is already locked: if we skipped the sampling the
    // edge would get out of phase, and a START held from before would be read as a new press.
    u32 pressedKeys = W1_GetPressedKeys();

    if (megaEvolveFlag != ALREADY_MEGA &&
        megaEvolveFlag != OTHER_MEGA)
    {
        if ((pressedKeys & KEY_START) == KEY_START)
        {
...
#if DYNAMIC_SPEED || EXPAND_ABILITIES || ADD_MEGA_EVOLUTION || EXPAND_ITEMS
...
extern "C" void THUMB_BRANCH_BattleField_InitCore(BattleFieldExt * battleField, WEATHER weather) {
    ...
    sys_memset(battleField->entrySlots, BATTLE_MAX_SLOTS, BATTLE_MAX_SLOTS);
...
extern "C" BattleFieldExt * THUMB_BRANCH_BattleField_Init(u16 heapID) {
    ...
    // The third argument is the "zero" flag, so the explicit memset is not needed.
    if (!g_BattleField) {
    ...
    // W1: the BattleUpgrade is RESIDENT → its globals survive between battles (on W2 the DLL
    // reloads every time and loadedDlls[] restarts clean for free). The cleanup existed only in
    // BattleField_Free, hooked to a fragile veneer. Here we are at battle start: idempotent.
    FreeLoadedDlls();

    return g_BattleField;
}

extern "C" void THUMB_BRANCH_BattleField_Free(void * ptr) {
    // W1: here the ESDB "BattleField_Free" (0x21EF9A1) lands on the VENEER of GFL_HeapFree
    // (0x21ef9a0: ldr r3,[..]; bx r3 -> 0x020307C8), called ONLY in the battle-teardown
    // (sub_2199D14 case 3, XREF[1] from 0x21b65f2). W1 does NOT have a real BattleField_Free, so
    // we use this point to free the effect-DLLs (which otherwise stay and fragment
    // the 141KB heap -> 2nd battle HEAP FULL). We do FreeLoadedDlls() AND THEN the
    // ORIGINAL free (0x020307C8, ARM) so as not to break the free of ov93.
    FreeLoadedDlls();
    // g_BattleField is allocated on the heap, so it must be returned. We use W1's REAL
    // free (0x020307C8, ARM). The `!= ptr` check avoids the double free in case the
    // teardown passes our very pointer.
    if (g_BattleField && (void*)g_BattleField != ptr) {
        ((void(*)(void*))0x020307C8)(g_BattleField);
    }
    g_BattleField = nullptr;
    ((void(*)(void*))0x020307C8)(ptr);
}
...
extern "C" u32 THUMB_BRANCH_SAFESTACK_ServerEvent_CalcDamage(ServerFlow * serverFlow, BattleMon * attackingMon, BattleMon * defendingMon,
    ...
    if (finalDamage) {
        ...
        // W1 FIX: ServerFlow_IsCompetitorScenarioMode has an unknown W1 address (divergent codegen
        // wall) and we jumped into it → freeze. It returns true in normal play (gates the damage
        // roll 85-100%), so stub to 1: roll applied in normal play, not in debug. Fixed.
        if (!MainModule_GetDebugFlag() && 1 /*ServerFlow_IsCompetitorScenarioMode(serverFlow)*/) {
            u32 damageRoll = 85;
            if (!battleDebugMode) {
...
extern "C" void THUMB_BRANCH_ServerControl_ApplyDamageToRecover(ServerFlow* serverFlow, BattleMon* attackingMon, MoveParam* moveParam, PokeSet* targetSet) {
    // W1 FIX: W1's ServerFlow (SDK 2010) is 4 bytes shorter than W2 → HEManager is at
    // serverFlow+0x1d74 (W2: 0x1d78). The header is W2 → &serverFlow->HEManager would point to
    // +0x1d78 (off by 4) and PushState/PopState would corrupt memory → freeze.
    // Verified by disasm: W1 vanilla @0x21c1500 uses 0x1d74, W2 @0x21a4fc0 uses 0x1d78.
    // W1 FIX: HEManager is at serverFlow+0x1d74 on W1 (W2 header: 0x1d78) — SDK 2010 struct divergence.
    // Verified by disasm: W1 vanilla @0x21c1500 uses 0x1d74, W2 @0x21a4fc0 uses 0x1d78.
    u32* heManager = (u32*)((char*)serverFlow + 0x1d74);
    ...
    for (BattleMon* currentMon = PokeSet_SeekNext(targetSet); currentMon; currentMon = PokeSet_SeekNext(targetSet)) {
        ...
        if (checkDamageFlag) {
            ...
            u32 HEID = HEManager_PushState(heManager);
            ServerEvent_ApplyDamageToRecover(serverFlow, attackingMon, currentMon);
            HEManager_PopState(heManager, HEID);
...
extern "C" void THUMB_BRANCH_ServerEvent_AfterSwitchInPrevious(ServerFlow* serverFlow) {
    ...
    // W1: EVENT_SWITCH_IN_PRIO is not registered by any handler on W1; its dispatch
    // chain (via ov95) reads the HP of a NULL mon on switch-in → freeze (GDB diagnosis Jul 2).
    // Removed: functional no-op on W1, eliminates the crash.
...
MoveEventAddTableExt moveEventAddTableExt[] = {
    {MOVE_RAPID_SPIN, EventAddRapidSpinUpdated, nullptr},
    {MOVE_DEFOG, EventAddDefogUpdated, nullptr},
    {MOVE_FLYING_PRESS, EventAddFlyingPress, nullptr},
    {MOVE_MAT_BLOCK, EventAddMatBlock, nullptr},
    {MOVE_ROTOTILLER, EventAddRototiller, nullptr},
    {MOVE_STICKY_WEB, EventAddStickyWeb, nullptr},
    {MOVE_FELL_STINGER, EventAddFellStinger, "Moves/FellStinger"},   // runtime DLL — pinned (IsPinnedDll) so it never moves
    {MOVE_PHANTOM_FORCE, EventAddShadowForce, nullptr},
    {MOVE_TRICK_OR_TREAT, EventAddTrickOrTreat, nullptr},
    {MOVE_ION_DELUGE, EventAddIonDeluge, nullptr},
    {MOVE_FOREST_S_CURSE, EventAddForestsCurse, nullptr},
    {MOVE_FREEZE_DRY, EventAddFreezeDry, nullptr},
    {MOVE_PARTING_SHOT, EventAddPartingShot, nullptr},
    {MOVE_TOPSY_TURVY, EventAddTopsyTurvy, nullptr},
    {MOVE_CRAFTY_SHIELD, EventAddCraftyShield, nullptr},
    {MOVE_FLOWER_SHIELD, EventAddRototiller, nullptr},
    {MOVE_GRASSY_TERRAIN, EventAddGrassyTerrain, nullptr},
    {MOVE_MISTY_TERRAIN, EventAddMistyTerrain, nullptr},
    {MOVE_ELECTRIFY, EventAddElectrify, nullptr},
    {MOVE_FAIRY_LOCK, EventAddSpiderWeb, nullptr},
    {MOVE_KINGS_SHIELD, EventAddKingsShield, nullptr},
    {MOVE_SPIKY_SHIELD, EventAddSpikyShield, nullptr},
    {MOVE_VENOM_DRENCH, EventAddVenomDrench, nullptr},
    {MOVE_POWDER, EventAddPowder, nullptr},
    {MOVE_MAGNETIC_FLUX, EventAddMagneticFlux, nullptr},
    {MOVE_HAPPY_HOUR, EventAddPayDay, nullptr},
    {MOVE_ELECTRIC_TERRAIN, EventAddElectricTerrain, nullptr},

    {MOVE_PSYCHIC_TERRAIN, EventAddPsychicTerrain, nullptr},

    {MOVE_PLASMA_FIST, EventAddPlasmaFist, nullptr},

    {MOVE_INFERNAL_PARADE, EventAddHex, nullptr},
    {MOVE_CEASELESS_EDGE, EventAddCeaselessEdge, nullptr},
};
...
PosEffectEventAddTableExt posEffectEventAddTableExt[] = {
    {POSEFF_ION_DELUGE, EventAddPosIonDeluge, nullptr},
    {POSEFF_CRAFTY_SHIELD, EventAddPosCraftyShield, nullptr},
    {POSEFF_ELECTRIFY, EventAddPosElectrify, nullptr},
    {POSEFF_KINGS_SHIELD, EventAddPosKingsShield, nullptr},
    {POSEFF_SPIKY_SHIELD, EventAddPosSpikyShield, nullptr},
    {POSEFF_POWDER, EventAddPosPowder, nullptr},
};
...
// W1 new-move animations decoupling: REAL moveID of the new move in progress (0 = no
// redirection). Written by MoveEffect_Start, read by the MoveAnimLoadW1 veneer on the if branch of FUN_021f9518.
extern "C" { u16 g_animMoveID = 0; }
extern "C" void THUMB_BRANCH_SAFESTACK_BattleViewCmd_MoveEffect_Start(BtlvCore * btlCore, u32 attackingPos, u32 targetPos, u16 moveID, u32 moveTarget, u32 effectIndex, u8 zero) {
    ...
    // W1 NEW-MOVE ANIMATIONS — DECOUPLING (editable without corruption).
    // Discovery (live debug): the W1 animation system corrupts the battle view when a moveID
    // >= 561 passes through it (the effect-layer/2nd loader loads garbage a067 → overlapped sprite),
    // regardless of the branch/archive. The forced Pound is clean because it maps everything to moveID 1 (<561).
    // Here we decouple like W2 does (but tailored to W1): the SYSTEM sees Pound (moveID 1 = safe, so
    // the effect-layer loads Pound's effect, no garbage), but we store the REAL moveID in
    // g_animMoveID; a hook on the loader (FUN_021f9518 if branch, @0x21f95de) redirects ONLY the main
    // visual load to a066[g_animMoveID]. Result: new move = Pound timing/effect, but a066[moveID]
    // EDITABLE visual via FrostsGen5Editor. See the MoveAnimLoadW1 hook in BattleEngine.S.
    if (moveID >= FIRST_BATTLE_ANIMATION_ID) {
        g_animMoveID = moveID;   // remember the real move for the loader hook
        moveID = MOVE_POUND;     // the system sees Pound (<561) → no corruption
    } else {
        g_animMoveID = 0;        // vanilla / battle-anim: no redirection
    }
    // W1 FIX: BtlvCore.btlvScu is at btlCore+0x17c on W1 (W2 header: 0x1c0) — SDK 2010 struct divergence.
    // Verified by disasm: W1 vanilla @0x21ea0c0 uses 0x17c (0x5f<<2), W2 @0x21cfca0 uses 0x1c0 (7<<6).
    BtlvScu* btlvScu = *(BtlvScu**)((char*)btlCore + 0x17c);
    CMD_ACT_MoveAnimStart(btlvScu, attckViewPos, defViewPos, moveID, moveTarget, effectIndex, zero);
}
